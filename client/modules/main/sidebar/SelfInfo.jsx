...
class SelfInfo extends Component {
    ...
    changePassword = async () => {
        const [err] = await fetch('changePassword', {
            oldPassword: encodeRSA(this.oldPassword.getValue()),
            newPassword: encodeRSA(this.newPassword.getValue()),
        });
        if (!err) {
    ...
    changeUsername = async () => {
        ...
        const [err] = await fetch('changeUsername', {
            newUserName: this.myUsername.getValue(),
        });
        if (!err) {
            Message.success('修改姓名成功');
            action.setUserName(this.myUsername.getValue());
        }
    }
    render() {
        const { visible, onClose, avatar, username, primaryColor } = this.props;
                ...
                <div className="content">
                                ...
                                cropper ?
                                    <div className="cropper">
                                        <Cropper
                                            ...
                                            aspectRatio={1}
                                        />
                                        <Button onClick={this.changeAvatar}>保存</Button>
                    ...
                    <div>
                        ...
                        <div className="change-username">
                            <Input ref={i => this.myUsername = i} type="text" value={username} placeholder={username} />
                            <Button onClick={this.changeUsername}>保存</Button>
                        </div>
                    </div>
                    <div>
                        <p>修改密码</p>
