...
class ChatInput extends Component {
    ...
    sendTextMessage = () => {
        ...
        if (!focus) {
            return Message.error('请先选中联系人,否则无法知道发给谁呢');
        }
    ...
    addSelfMessage = (type, content, filename) => {
    ...
    sendFileMessage = (file, fileType = 'file') => {
        ...
        console.log('sendFileMessage:', url, file);
        const _id = this.addSelfMessage(fileType, url, file.filename);
        const { userId, focus } = this.props;
        const al = async () => {
            const pkgId = await uploadFile(
                file.result,
                `FileMessage_${Date.now()}.${ext}`,
                file.filename,
                (info) => {
                    action.updateSelfMessage(focus, _id, { percent: info.total.percent, loading: info.total.loading, filename: info.total.filename, filesize: info.total.filesize });
                },
            );
            // console.log('file############', pkgId);
            this.sendMessage(_id, fileType, pkgId, focus);
        };
        al();
    };

    handleSelectImageFile = async () => {
        if (!this.props.connect) {
            return Message.error('发送消息失败, 您当前处于离线状态');
        }
    ...
    handleSelectVideo = async () => {
        if (!this.props.connect) {
            return Message.error('发送消息失败, 您当前处于离线状态');
        }
        const file = await readDiskFile('blob', 'audio/mp4,video/mp4,video/mpeg');
        if (!file) {
            return;
        }
        this.sendFileMessage(file, 'video');
    };
    handleSelectAudio = async () => {
        if (!this.props.connect) {
            return Message.error('发送消息失败, 您当前处于离线状态');
        }
        const file = await readDiskFile('blob', 'audio/mpeg');
        if (!file) {
            return;
        }
        console.log('file:', file);
        this.sendFileMessage(file, 'audio');
    };
    sendHuaji = async () => {
        const huaji = getRandomHuaji();
        console.log('huaji:', huaji);
    ...
    handlePaste = (e) => {
        ...
        console.log('粘贴...', types, e, e.clipboardData);
    ...
    featureDropdown = (
        <div className="feature-dropdown">
            <Menu onClick={this.handleFeatureMenuClick}>
                ...
                <MenuItem key="video">发送视频</MenuItem>
                <MenuItem key="audio">发送语音</MenuItem>
                <MenuItem key="loc">发送位置</MenuItem>
