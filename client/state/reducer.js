...
function reducer(state = initialState, action) {
    ...
    case 'SetUser': {
        let newState = state;
        console.log('SetUser:', state, action.user.linkmans);
        if (state.getIn(['user', '_id']) === undefined) {
            newState = newState
                .set('user', immutable.fromJS(action.user))
                .set('focus', state.get('focus') || (action.user.linkmans.length > 0) ? action.user.linkmans[0]._id : '');
        } else {
                ...
                linkmans
                    ...
                    .unshift(linkman
                        .update('messages', (messages) => {
                            ...
                            // console.log('AddLinkmanMessage:', JSON.stringify(newMessages));
                            return newMessages;
                        })
                        .set('unread', unread))
            ));
    ...
    case 'UpdateSelfMessage': {
        ...
        return state.updateIn(['user', 'linkmans', linkmanIndex, 'messages'], (messages) => {
            ...
            console.log('messages: ', messages, messageIndex, action.messageId, action.message);
    ...
    case 'SetUserName': {
        return state.setIn(['user', 'username'], action.username);
    }
    case 'ChangePanel': {
        console.log('ChangePanel, CurrentPanelName:', action.panelName);
        console.log('ChangePanel', state, action.panelName, action.keys, immutable.fromJS(action.value));
        return state.set('panelName', action.panelName);
    }
    case 'SetEbook': {
        console.log('SetEbook', action.value);
        return state.setIn(action.keys, immutable.fromJS(action.value));
    }
    case 'SetEbookOrg': {
        console.log('SetEbookOrg', action.value);
        return state.setIn(action.keys, immutable.fromJS(action.value));
    }
